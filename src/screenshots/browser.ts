 ...
 * 1. explicit path (plan/config) or STORYOPS_CHROMIUM_PATH (legacy: EDITORIAL_CHROMIUM_PATH),
...
export async function resolveChromiumExecutable(explicit?: string): Promise<string | undefined> {
  const candidate = explicit ?? process.env.STORYOPS_CHROMIUM_PATH ?? process.env.EDITORIAL_CHROMIUM_PATH;
